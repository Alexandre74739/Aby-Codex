...
import { concepts, Concept } from '../../data/concept';
import './Game.scss';
...
export default function Game() {
  const [activeId, setActiveId] = useState<string | null>(concepts[0].id); // Premier concept actif par défaut
  const activeConcept = concepts.find(c => c.id === activeId) ?? null;

  return (
    <section className="game-manifesto">
      {/* Arrière-plan décoratif épuré */}
      <div className="game-overlay" />

      <div className="game-container">
        
        {/* HERO SECTION DU MANIFESTE */}
        <header className="game-header">
          <div className="header-meta">Exploration des courants</div>
