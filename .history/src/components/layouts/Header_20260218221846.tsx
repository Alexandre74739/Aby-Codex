    ...
    <header className={`main-header ${isScrolled ? 'scrolled' : ''}`}>
      ...
      <div className={`mobile-drawer ${menuOpen ? 'open' : ''}`}>
        <div className="drawer-content">
          {navLinks.map((link) => (
            <Link key={link.path} to={link.path} className={`drawer-link ${location.pathname === link.path ? 'active' : ''}`}>
              {link.label}
            </Link>
          ))}
          <Link to="/diagnostic" className="drawer-link diag-special">DIAGNOSTIC</Link>
        </div>
      </div>
    </header>
  );
